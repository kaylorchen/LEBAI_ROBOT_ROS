...
void WriteData(const uint8_t number, const uint32_t can_id, const uint8_t *data, const uint8_t size) {
  int fd = -1;
  switch (number) {
    case 0:fd = can0;
      break;
...
void InitDevice(void){
  ...
  /********************* 过滤规则设置 *********************/
  /**
   * 中菱电机默认是ID是1， 我们这里过滤器使用过滤0x581（SDO反馈包）和0x701（心跳包）
   * **/
...
const double kWheelDistance = 229.8/1000;
...
void Callback(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  ROS_INFO("linear: %lf m/s, angular: %lf rad/s, left = %d rpm, right = %d rpm", linear, angular, rpm_l, rpm_r);
